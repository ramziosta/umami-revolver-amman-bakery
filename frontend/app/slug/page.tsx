...
const ItemCard = ({ slug, item }: { slug: string; item: Item }) => {
    ...
    return (
        <Link href={`/menu/${slug}/item/${item.id}`}>
            <Card className="group cursor-pointer elegant-shadow hover:shadow-2xl transition-all duration-500 overflow-hidden border-0 bg-white">
                ...
                <CardContent className="p-6 bg-white group-hover:bg-umami-white transition-colors duration-300">
                    <p className="text-umami-black text-lg  leading-relaxed mb-2">
                        {item.itemDescription || 'No description available'}
                    </p>

                    {item.itemPrice && typeof item.itemPrice === "number" && (
                        <p className="text-umami-black font-semibold">
                            Price: {item.itemPrice} JOD
                        </p>
                    )}

                    {item.itemPrices && item.itemPrices.length > 0 && (
                        <div className="mt-2 text-umami-black">
                            <h4 className="">Sizes & Prices:</h4>
                    ...
                    {item.itemPrice && typeof item.itemPrice === "object" && (
                        <div className="mt-2 text-umami-black">
                            <h4 className="">Options & Prices:</h4>
                            <ul className="list-disc pl-5">
...
export default function CategoryPage({ params }: Params) {
    ...
    if (!category) {
        return (
            <div className="min-h-screen bg-umami-white flex items-center justify-center">
                <div className="text-center">
                    <h1 className="text-4xl  text-umami-black mb-4">Category Not Found</h1>
                    <p className="text-umami-black">The category "{slug}" could not be found.</p>
                    <Link href="/categories" className="mt-4 inline-block bg-umami-black text-white px-6 py-2 rounded hover:bg-opacity-90 transition-colors">
                        Back to Categories
                    </Link>
                </div>
            </div>
        ...
        <div className="min-h-screen bg-umami-white">
            ...
            {category.cupcakeQuantityOptions && category.cupcakeQuantityOptions.length > 0 && (
                <section className="container mx-auto px-4 pb-8">
                    <h2 className="text-2xl  mb-2">Cupcake Options</h2>
                    <ul className="list-disc pl-5">
                ...
                <div className="container mx-auto px-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                        {category.items && Array.isArray(category.items) && category.items.length > 0 ? (
                            ...
                            <div className="col-span-full text-center py-8 text-umami-black">
